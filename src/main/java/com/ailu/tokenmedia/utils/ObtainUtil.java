...
public class ObtainUtil {
	...
	/**
	 * 使用密钥和Http-Body中的内容进行UTF-8编码生成字节流， 然后对字节流进行MD5加密生成字节流，将字节流转换成字符串，
	 * 并将’-’字符替换成空字符， 最后截取从字符串14位开始截取4个字符转小写后作为检验码。
	 *
	 * @param body
	 *            body就是Http Request Body 中的内容
	 * @return 4位的校验码
	 */
	public String genCheckSum(String body) {
		String key = body + accountKey;
		System.out.println(key);
		String strRead = MD5Util.getMD5(key);
		System.out.println("MD5: " + strRead);
		String count = strRead;
		count = count.replace("-", "");
		System.out.println("checksum: " + count.substring(14, 18));
